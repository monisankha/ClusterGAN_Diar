...
def main(segments, labels, rttmpath, rttm_channel):
  ...
  x = file.readlines()
  for i in range(len(x)):
    seg, reco, start, end = x[i].strip().split()
    try:
  ...
  for reco_line in merged_segs:
    ...
    for i in range(1, len(segs)):
      ...
      fid.write("SPEAKER {0} {1} {2:7.3f} {3:7.3f} <NA> <NA> {4} <NA> <NA>\n".format(
        reco1, rttm_channel, float(start), float(end)-float(start), label))
  fid.close()
